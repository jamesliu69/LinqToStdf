...
namespace Stdf
{
	...
	public struct RecordType : IComparable, IComparable<RecordType>, IEquatable<RecordType>
	{
		...
		/// </summary>
		public override int GetHashCode() => (Type << 24) | Subtype;
...
#region IComparable<RecordType> Members
		...
		{
			int value = Type.CompareTo(other.Type);

			if(value == 0)
			{
				value = Subtype.CompareTo(other.Subtype);
			}
			return value;
		}
...
#region Operators
		...
		///     Equality operator
		/// </summary>
		public static bool operator ==(RecordType first, RecordType second) => first.Equals(second);
