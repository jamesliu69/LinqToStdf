...
namespace Stdf.RecordConverting
{
    ...
    internal class ConverterGenerator
	{
				...
				foreach(KeyValuePair<FieldLayoutAttribute, PropertyInfo> pair in fields)
				{
					...
					if((pair.Value != null) && _Fields.Contains(pair.Value.Name))
					{
						...
						if(pair.Key is FlaggedFieldLayoutAttribute op)
						{
							...
							PropertyInfo prop = fields[op.FlagIndex].Value;
						...
						else
						{
							...
							{
								PropertyInfo prop = fields[dep.DependentOnIndex].Value;
		...
		private static List<KeyValuePair<FieldLayoutAttribute, PropertyInfo>> GetFieldLayoutsAndAssignments(Type recordType)
		{
			//get the list
			IOrderedEnumerable<FieldLayoutAttribute> attributes = from a in recordType.GetCustomAttributes(typeof(FieldLayoutAttribute), true).Cast<FieldLayoutAttribute>() orderby a.FieldIndex select a;
			...
			for(int i = 0; i < list.Count; i++)
			{
				...
				{
					throw new NonconsecutiveFieldIndexException(recordType);
				}
			}
			IEnumerable<KeyValuePair<FieldLayoutAttribute, PropertyInfo>> withPropInfo = from l in list select new KeyValuePair<FieldLayoutAttribute, PropertyInfo>(l, l.RecordProperty == null ? null : recordType.GetProperty(l.RecordProperty));
		...
		private CodeNode GenerateAssignment(KeyValuePair<FieldLayoutAttribute, PropertyInfo> pair)
		{
			...
			if(pair.Value != null)
			{
				List<CodeNode> assignmentNodes = new List<CodeNode>();
		...
		private CodeNode GenerateArrayAssignment(KeyValuePair<FieldLayoutAttribute, PropertyInfo> pair)
		{
			...
			ReadTypeNode readNode        = new ReadTypeNode(fieldType.MakeArrayType(), lengthIndex, isNibbleArray);
			BlockNode    assignmentBlock = null;
