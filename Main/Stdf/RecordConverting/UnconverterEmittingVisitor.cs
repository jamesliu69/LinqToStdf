...
{
	internal class UnconverterEmittingVisitor : CodeNodeVisitor
	{
		...
		private LocalBuilder _StartedWriting;
		private LocalBuilder _Writer;
		...
		public override CodeNode VisitUnconverterShell(UnconverterShellNode node)
		{
			...
			LocalBuilder content = ILGen.DeclareLocal<byte[]>();
			ILGen.Stloc(content);
			ILGen.Ldloc(content);
		...
		public override CodeNode VisitCreateFieldLocalForWriting(CreateFieldLocalForWritingNode node)
		{
			...
			LocalBuilder local = ILGen.DeclareLocal(node.LocalType);
		...
		public override CodeNode VisitWriteField(WriteFieldNode node)
		{
			...
			LocalBuilder hasValueLocal = ILGen.DeclareLocal<bool>();
			ILGen.Ldc_I4_0();
			ILGen.Stloc(hasValueLocal);

			//declare a bunch of labels
			Label decideLabel      = ILGen.DefineLabel();
			Label skipWritingLabel = ILGen.DefineLabel();
			Label doWriteLabel     = ILGen.DefineLabel();

			//if we have a property, we need to read from it
			if(node.Property != null)
			{
				//get the field local
				LocalBuilder fieldLocal = _FieldLocals[node.FieldIndex];
				...
				if(node.Property.PropertyType.IsValueType)
				{
					...
					if(node.Property.PropertyType == nullable)
					{
						...
						LocalBuilder nullableLocal = ILGen.DeclareLocal(nullable);
			...
			if(node.OptionalFieldIndex.HasValue)
			{
				...
				LocalBuilder optionalLocal = _FieldLocals[node.OptionalFieldIndex.Value];
				Label        skipOptField  = ILGen.DefineLabel();
		...
		public override CodeNode VisitValidateSharedLengthLocal(ValidateSharedLengthLocalNode node)
		{
			...
			LocalBuilder arrayLocal  = _FieldLocals[node.ArrayFieldIndex];
			LocalBuilder lengthLocal = _FieldLocals[node.LengthFieldIndex];
			...
			Label compareLength = ILGen.DefineLabel();
		...
		public override CodeNode VisitSetLengthLocal(SetLengthLocalNode node)
		{
			...
			LocalBuilder arrayFieldLocal  = _FieldLocals[node.ArrayFieldIndex];
			LocalBuilder lengthFieldLocal = _FieldLocals[node.LengthFieldIndex];

			//TODO: this code is duplicated in VisitValidateSharedLengthLocal
			Label notNull     = ILGen.DefineLabel();
			Label storeLength = ILGen.DefineLabel();
