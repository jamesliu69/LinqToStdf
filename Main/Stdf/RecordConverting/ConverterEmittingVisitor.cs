...
{
	internal class ConverterEmittingVisitor : CodeNodeVisitor
	{
		public ILGenerator ILGen;
		public Type        ConcreteType;
		public bool        EnableLog = false;

		private          LocalBuilder                  _ConcreteRecordLocal;
		private          LocalBuilder                  _Reader;
		private          bool                          _InFieldAssignmentBlock;
		private          bool                          _InFieldAssignment;
		private          Label                         _EndLabel;
		private          Label                         _SkipAssignmentLabel;
		private readonly Dictionary<int, LocalBuilder> _FieldLocals = new Dictionary<int, LocalBuilder>();

		private LocalBuilder _FieldLocal;
		...
		private static readonly MethodInfo _EnsureConvertibleToMethod = typeof(UnknownRecord).GetMethod("EnsureConvertibleTo", typeof(StdfRecord));
		...
		private static readonly MethodInfo _DisposeMethod = typeof(IDisposable).GetMethod("Dispose", new Type[0]);
		...
		public override CodeNode VisitFieldAssignmentBlock(FieldAssignmentBlockNode node)
		{
			...
			try
			{
				...
				CodeNode visitedBlock = VisitBlock(node.Block);
		...
		private static readonly MethodInfo _SkipRawMethod = typeof(BinaryReader).GetMethod("Skip", typeof(int));
		...
		private readonly Dictionary<Type, MethodInfo> _SkipTypeMethods = new Dictionary<Type, MethodInfo>();
		...
		private static readonly MethodInfo _ReadFixedStringMethod = typeof(BinaryReader).GetMethod("ReadString", typeof(int));
		...
		private readonly Dictionary<Type, MethodInfo> _ReadTypeMethods = new Dictionary<Type, MethodInfo>();
		...
		private static readonly MethodInfo _AtEndOfStreamMethod = typeof(BinaryReader).GetProperty("AtEndOfStream").GetGetMethod();
		...
		public override CodeNode VisitFieldAssignment(FieldAssignmentNode node)
		{
			...
			try
			{
				...
				Label assignmentCompleted = ILGen.DefineLabel();
		...
		public override CodeNode VisitSkipArrayAssignmentIfLengthIsZero(SkipArrayAssignmentIfLengthIsZeroNode node)
		{
			...
			LocalBuilder lengthLocal = _FieldLocals[node.LengthIndex];
