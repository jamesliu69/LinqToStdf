	...
	public class Plr : StdfRecord
	{
		...
		internal static UnknownRecord ConvertFromPlr(StdfRecord record, Endian endian)
		{
			...
			using(MemoryStream stream = new MemoryStream())
			{
				...
				{
					throw new InvalidOperationException(string.Format(Resources.ArrayTooLong, ushort.MaxValue, 1, typeof(Plr)));
				}
				...
				else if(fieldsWritten)
				{
					...
					Array.ForEach(arr, delegate(string a) { a = ""; });
				...
				else if(fieldsWritten)
				{
					...
					Array.ForEach(arr, delegate(byte a) { a = byte.MinValue; });
				...
				else if(fieldsWritten)
				{
					...
					Array.ForEach(arr, delegate(ushort a) { a = ushort.MinValue; });
