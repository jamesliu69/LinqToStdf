	...
	public partial class StdfFile
	{
		...
		private class RewindableByteStream
		{
			...
			public RecordHeader? ReadHeader(Endian endian)
			{
				...
				//convert to ushort
				ushort length = BitConverter.ToUInt16(_Buffer, 0);
			...
			public void Flush()
			{
				...
				if(_Rewound > 0)
				{
					...
					byte[] data = _MemoizedData.ToArray();
			...
			public byte[] DumpDataToCurrentOffset()
			{
				if(_Rewound > 0)
				{
					long position = _MemoizedData.Position;
					_MemoizedData.Seek(0, SeekOrigin.Begin);
					long   length = _MemoizedData.Length - _Rewound;
